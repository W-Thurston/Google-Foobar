...
# debut on the galactic stage - but in order to make a grand entrance, she needs a grand staircase! 
...
# You can buy different amounts of the different types of bricks (for example, 3 little pink bricks, 
# or 5 blue lace bricks). Commander Lambda wants to know how many different types of staircases 
...
# For example, when N = 3, you have only 1 choice of how to build the staircase, with the 
# first step having a height of 2 and the second step having a height of 1: (# indicates a brick)
...
# the number of different staircases that can be built from exactly n bricks. n will 
# always be at least 3 (so you can have a staircase at all), but no more than 200, because 
...
def answer(n):
	m = [[0 for x in range(n+1)] for i in range(n+1)]
	m[0][0] = 1 # base case
